...
Read by the Codex GitHub review integration (and the Claude reviewer in `.github/workflows/claude-review.yml`) to focus PR review on serious findings. Mirrored in `AGENTS.md` so both reviewers apply the same bar. **P0 = block merge; P1 = strong objection; everything else = nit at most.**
...
| Change the Kapital & Auszahlungen full-page lifecycle surface (`/kapital`: kicker + H1 + filter chips + lifecycle chart + § 1 Wendepunkte table). Dual-source — renders for both compare and combine modes | `src/features/kapital/KapitalPage.tsx` (page); sub-components `KapitalFilterChips.tsx`, `KapitalWendepunkteTable.tsx`; pure helpers `kapitalFilters.ts` (per-mode chip option builders), `wendepunkte.ts` (turning-point row builder) |
| Change the Vergleich compare-mode surface (lead + rendite-annahme chip strip + comparison picker + § 1 6-product table + § 2 pro/contra grid). Compare-mode-only — combine-mode routes to `MeinPlanPage` | `src/features/vergleich/VergleichPage.tsx` (page); sub-components `VergleichRenditeStrip.tsx`, `VergleichComparisonTable.tsx`, `VergleichProContraGrid.tsx`; copy in `src/content/proContraCopy.ts` + `src/features/vergleich/productTaglines.ts` |
| Change the per-product breakdown surface (`/vergleich/details`: header + one card per visible product, with Ansparphase / Mit `{retirementAge}` / Im Alter sections + Verfügbar-ab footer). Compare-mode-only drill-in from VergleichPage; combine-mode renders an EmptyState | `src/features/vergleich-detail/VergleichDetailPage.tsx` (page); sub-components `VergleichDetailCard.tsx`, `VergleichDetailCardSection.tsx`; pure helpers `vergleichDetailRows.ts` (per-product row builder), `vergleichDetailAvailability.ts` (Verfügbarkeit text); content registry `src/content/productAvailabilityCopy.ts` (`AvailabilityEntry.label` must **not** start with `"ab "` — the footer prepends `"Verfügbar ab "` in both `VergleichDetailCard.tsx` and its `PrintReport.tsx` mirror; the mirror uses different CSS classes and must be updated separately) |
| Change the live Monte-Carlo panel on `/methode` (relocated from the legacy Vergleich pane in PR 9) | `src/features/methode/MethodeMonteCarloSection.tsx` (live wrapper that subscribes to compare-mode `useSimulationResult`) + `src/features/results/MonteCarloPanel.tsx` (chart) |
| Extend the printable A4 report (compare-mode + combine-mode). Disclaimer is the LITERAL first child of `#print-report` — adding a section means inserting AFTER the `.pr-disclaimer-top` block, never before. Compare-mode mirrors `/vergleich/details` and `/methode`; combine-mode adds Zusammensetzung / Wendepunkte / Vertrag im Detail + Methode | `src/features/results/PrintReport.tsx` + `PrintReport.css`; pure row builders in `src/features/results/printReportRows.ts` (reuses `vergleichDetailRows.ts`, `wendepunkte.ts`, `breakEvenSeries.ts`) |
| Tune viewport chart density (axis label visibility, axis width, margins, callout font sizes) for any Recharts surface | `src/ui/charts/useChartDensity.ts` — width-based token provider (`useChartDensity(chartWidth)` returns Recharts-friendly tokens). Adopted by `BreakEvenChart`, `FeeDragChart`, `MonteCarloPanel` |
...
- **Recharts canonical reference** (`BreakEvenChart.tsx`): when adding a new Recharts surface to one of the Sober D pages (Vergleich, Kapital, Vertrag-Detail, Methode, Mein Plan), use `BreakEvenChart.tsx` as the template. Three recurring tsc-b errors to avoid: (1) import `TooltipContentProps` (not `TooltipProps`) from `recharts/types/component/Tooltip`; (2) pass the tooltip as a render prop (`content={(props) => <MyTooltip {...props} />}`) not a JSX element; (3) pass `useFeedbackTarget` an object `{ id, label, precision }` not a bare string; spread `{ targetProps }` onto the host element. Skipping any of these causes `tsc -b` failures caught only by `npm run verify`, not `npx tsc --noEmit`.
...
- **`PRODUCT_REGISTRY` is the source of truth for product identity.** `ProductId` is derived from each product's `metadata.id` literal. Adding a product is local to `engine/products/` + per-product domain types + one registry entry. Use `getProductMeta(id)` in UI instead of local color/order maps — note it returns `ProductManifestEntry | undefined`, so every call-site must null-guard even when the product id is known (`meta?.label ?? result.productId`). The UI sibling `productUiRegistry.tsx` (`Record<ProductId, ProductUiEntry>`) drives `InputsPanel` dispatch; engine code never imports React.
...
- **Fair-comparison invariant (compare-mode only).** In compare-mode (singleton state, `simulateRetirementComparison` path), ETF and insurance always invest `bavFunding.monthlyNetCost` — the same net cash the user pays for bAV. There is no "custom amount" toggle in compare-mode: every product invests the same net. `ScenarioAssumptions.contributionInput` is **not** such a toggle — it changes only *which quantity the user types*, not whether the amounts stay equal. In `{ kind: 'avd-own' }` the user steers the AVD Eigenbeitrag (the statutory AVD thresholds apply to it, not to net cost) and `syncMonthlyContributions` derives the shared anchor from it, so all products still invest AVD's true net cost. Because the anchor is derived by a fixed-point iteration rather than in closed form, equality is a **numerical postcondition** with a bounded residual (measured worst case 0.0933 EUR/month at the pension BBG), not an exact identity — `src/utils/syncContributions.test.ts` pins it. The anchor must be re-derived inside the sync on every call; storing it lets it go stale and silently breaks this invariant. In **combine-mode** (workspace with per-instance arrays via `simulatePortfolio`), each ETF instance's `monthlyContribution` is honored via `BuildContextOverrides.etfMonthlyUserCostOverride` and each insurance instance's `monthlyContribution` is honored via `BuildContextOverrides.insuranceMonthlyUserCostOverride`; the invariant doesn't apply because users are modeling actual independent contracts. Each contract may also set `InstanceCommon.expectedReturn` (optional decimal ratio, −0.5 to 0.5): `scenarioForInstance` replaces the market return absolutely in all three scenarios, preserving scenario ids and labels, including the shared ETF allowance re-run. Clearing the field restores the scenario rate. Singleton projections strip it, so compare mode and its Monte Carlo keep one shared return; combine mode does not run Monte Carlo. AVD applies its existing safety allocation and glidepath to the overridden risky-market return.
...
- **`capitalInjections` and `costBasisInjections` have distinct single-responsibility semantics.** `capitalInjections` grows the pot (capital-only, no cost-basis implication). `costBasisInjections` extends the tracked cost basis only (no capital injection). `projectAccumulation` sums both into `totalContributionsBeforeFees` via one shared accumulator — populating *both* arrays with the same proceeds (as `portfolioTransfer.ts` formerly did for surrender-reinvest targets) double-counts the basis. For certified inbound transfers, `costBasisInjections` must be omitted entirely (`portfolioTransfer.test.ts:612` guards this invariant explicitly).
...
- **`returnScenarios[0]` is not necessarily 'basis'.** `defaultAssumptions.returnScenarios` orders them `[konservativ, basis, optimistisch]`. Never index by position when you need the basis scenario — always `find(s => s.id === 'basis') ?? returnScenarios[0]`. Hardcoding `[0]` picks `konservativ` (3 %) instead of `basis` (5 %), a 2 pp gap that compounds to hundreds of EUR/month over a 39-year projection. See `pickBasisScenario` in `src/app/recommender.ts:362–377` for the canonical pattern; `simulateContractDecision` in `src/app/optimiereVorsorge.ts` must pair baseline and applied on the same scenario.
- **`ProductResult` shape gap fix path.** When adding a field to `BaseProductResult` in `src/domain/results.ts`, the complete propagation checklist is: (1) `BaseProductResult` in `results.ts`, (2) `buildResult.ts` return object, (3) any downstream consumer (e.g., `portfolioCombine.ts`), (4) `synthesizeProductResult` in `src/app/recommenderCandidates/types.ts` — this factory uses an `as ProductResult` cast so `npx tsc --noEmit` misses missing fields; only `tsc -b` (i.e., `npm run verify`) catches it, (5) test fixtures that manually construct `ProductResult` literals. Missing step 4 causes silent `NaN` in downstream gain-ratio calculations. The same tsc-b-only catch applies to `BavFundingResult`: `src/app/recommenderCandidates/bav.ts` manually synthesizes a `BavFundingResult` with an implicit return type — new fields added to `BavFundingResult` in `src/domain/products/bav.ts` need zero/false defaults added there too.
...
- **Combine mode** — workspace with per-product instance arrays (`schemaVersion: 2`), baseline + what-if scenarios, transfer events (`certified` / `surrender_reinvest`; `partial_transfer` is not yet in the schema; `paid_up` is an instance status, not a transfer event type), cross-instance funding caps, shared §20 Abs. 9 EStG Sparerpauschbetrag, and household-level results via `combinePortfolio`. Architecture map in [`CONTEXT.md`](CONTEXT.md). Compare mode remains as the singleton path for the legacy share-URL / scenario-library surface.
