...
| Change the per-product breakdown surface (`/vergleich/details`: header + one card per visible product, with Ansparphase / Mit `{retirementAge}` / Im Alter sections + Verfügbar-ab footer). Compare-mode-only drill-in from VergleichPage; combine-mode renders an EmptyState | `src/features/vergleich-detail/VergleichDetailPage.tsx` (page); sub-components `VergleichDetailCard.tsx`, `VergleichDetailCardSection.tsx`; pure helpers `vergleichDetailRows.ts` (per-product row builder), `vergleichDetailAvailability.ts` (Verfügbarkeit text); content registry `src/content/productAvailabilityCopy.ts` |
