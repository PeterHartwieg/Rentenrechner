...
**Accumulation policy** (`accumulation.ts`): `AccumulationInput` carries the always-required fields (contributions, return, fees, etc.); the opt-in behaviors live on `policy?: AccumulationPolicy` — `yearlyReturn` (per-year return override; Standarddepot glidepath today, Monte Carlo plugs in here next), `vorabpauschale` (InvStG §18 annual accrual + cost-basis carryover), `initialCapital` (transferred starting balance for Riester→AVD and paid-up insurance phase 2), and `contributionGrowth` (Beitragsdynamik — see below). `buildResult` mirrors the shape minus `rules`, which it injects from `params.rules`. New extension policies should be added to `AccumulationPolicy` rather than as new top-level options.

**Beitragsdynamik** (`accumulation.ts`): `policy.contributionGrowth.annualRate` scales each year's `monthlyUserCost`, `monthlyProductContribution`, and `monthlyEmployerContribution` by `(1 + r)^yearIndex`. `acquisitionCostPct` is computed from the geometric-sum Beitragssumme (Versicherungs convention) so Abschlusskosten reflect the full contract horizon. Wired into ETF, bAV, and pAV (insurance) product simulators via `EtfAssumptions.annualContributionGrowthRate` / `BavAssumptions.annualContributionGrowthRate` / `InsuranceAssumptions.annualContributionGrowthRate` (default 0). For bAV, the §3 Nr. 63 / §1 SvEV cap and statutory §1a Abs. 1a subsidy are computed from year-1 inputs and held constant — an approximation, since BBG also grows ~2-3 %/year in practice.
...
Agent-readability refactor phases 0–11 complete. All 562 tests pass.
