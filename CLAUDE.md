...
| Tune viewport chart density (axis label visibility, axis width, margins, callout font sizes) for any Recharts surface | `src/ui/charts/useChartDensity.ts` — width-based token provider (`useChartDensity(chartWidth)` returns Recharts-friendly tokens). `BreakEvenChart` consumes it; `FeeDragChart` / `MonteCarloPanel` migration is a follow-up |
...
- **Vergleich pane dispatcher** (`Calculator.tsx:675–710`): the dispatch block uses BOTH positive branches (`vergleichPane === '<slug>'` → render the pane component) AND negative exclusions from three generic fallback charts (CapitalChart, PensionChart, BreakEvenChart). Adding a new pane requires all three: (1) add the slug literal to `VergleichPaneSlug` union in `src/features/results/vergleichPanes.ts`, (2) add it to `ALL_VERGLEICH_PANES` (gates `?pane=` URL deep-linking — slugs absent here are silently ignored by the URL-init `useEffect`), and (3) add `&& vergleichPane !== '<slug>'` to each of the three generic chart predicates at `Calculator.tsx:~683/692/699`. Missing any one of these three steps causes either silent URL-init failure or the new pane rendering behind or alongside generic charts.
- **Recharts canonical reference** (`BreakEvenChart.tsx`): when adding a chart to a new Vergleich pane, use `BreakEvenChart.tsx` as the template. Three recurring tsc-b errors to avoid: (1) import `TooltipContentProps` (not `TooltipProps`) from `recharts/types/component/Tooltip`; (2) pass the tooltip as a render prop (`content={(props) => <MyTooltip {...props} />}`) not a JSX element; (3) pass `useFeedbackTarget` an object `{ id, label, precision }` not a bare string; spread `{ targetProps }` onto the host element. Skipping any of these causes `tsc -b` failures caught only by `npm run verify`, not `npx tsc --noEmit`.
