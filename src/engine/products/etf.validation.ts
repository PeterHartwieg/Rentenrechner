...
import { CONTRIBUTION_GROWTH_BOUNDS } from '../../domain/validation/bounds'
...
export function validateEtf(etf: EtfAssumptions): boolean {
  ...
  if (!(VALID_PARTIAL_EXEMPTIONS as readonly number[]).includes(etf.equityPartialExemption)) return false
  return inRange(
    etf.annualContributionGrowthRate,
