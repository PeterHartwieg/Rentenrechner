...
describe('buildRouteHead — per-route metadata shape', () => {
  it.each(PUBLIC_ROUTE_IDS)(
    'route %s carries title, description, canonical, robots, OG, Twitter, JSON-LD',
    (routeId) => {
      ...
      expect(head.jsonLd).toBeDefined()
    },
  )

  it('emits WebApplication JSON-LD for the homepage and topic page', () => {
    expect(buildRouteHead('/').jsonLd['@type']).toBe('WebApplication')
...
describe('renderRouteHeadHtml — JSON-LD parses + visible-content alignment', () => {
  it.each(PUBLIC_ROUTE_IDS)('route %s emits exactly one valid JSON-LD block', (routeId) => {
