 ...
 *   - `RULES` is the registry of all active rules.
 *   - `runRules` runs all rules and flattens output.
 *
 * German copy templates (`renderAtom`, `AtomTemplate`) live in
 * `src/content/recommendationCopy.ts` — import from there for display needs.
 *
 ...
 *   `decisionLogic.ts` calls `runRules` internally and reshapes the atoms
