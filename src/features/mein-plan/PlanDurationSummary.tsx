 ...
 */
export function PlanDurationSummary({ rows, onEditSharedHorizon, onOpenKapital, retirementAge, targetMonthly }: PlanDurationSummaryProps) {
  ...
  const canShowAmounts = rows.some((row) => Number.isFinite(row.netMonthlyReal) && row.netMonthlyReal !== 0)
  return (
    <section className="plan-overview plan-duration-summary">
      ...
      {firstEnding && <div className="plan-overview__notice" role="note" data-testid="plan-duration-cutoff">
        <p><strong>Ab Alter {firstEnding.endAge} fällt {firstEnding.row.label} weg</strong>
          {canShowAmounts && firstEnding.row.status !== 'unknown' && Number.isFinite(firstEnding.row.netMonthlyReal)
            ? <> ({formatCurrency(firstEnding.row.netMonthlyReal)} pro Monat in heutigen Euro, Stand Rentenbeginn)</>
            : null}.
        ...
        <table className="plan-duration-summary__table" data-testid="plan-duration-checkpoints">
          ...
          <tbody>
            {rows.map((row) => <tr key={row.key}>
              ...
              <td className="plan-duration-summary__num">{canShowAmounts && row.status !== 'unknown' && Number.isFinite(row.netMonthlyReal) ? formatCurrency(row.netMonthlyReal) : '—'}</td>
              {ages.map((age) => {
