...
| **Transfer event** | A scheduled cash/capital transition between instances or out of an instance: `surrender_reinvest`, `partial_transfer`, `paid_up`. Type: `TransferEvent` in `src/domain/instances.ts`. Collected by `collectTransferEvents` in `portfolioTransfer.ts`. Storage backfill uses the same `transferEventKey` shape from `storage.ts`. |
