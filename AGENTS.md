...
| `src/engine/productManifest.ts` | `PRODUCT_MANIFEST` (ordered array of all product metadata), `getProductMeta(id)` helper. Single source of truth for product ids, labels, colors, and sort order. |
...
**Product manifest** (`productManifest.ts`): `PRODUCT_MANIFEST` is the single source of truth for product display metadata (id, label, shortLabel, color, order, lockedCapital, hasFees, hasEmployerContribution). Each product module in `src/engine/products/` exports a `metadata` constant; the manifest aggregates them. Use `getProductMeta(productId)` in UI code instead of local color/order maps.
