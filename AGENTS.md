...
| `src/engine/productRegistry.ts` | `PRODUCT_REGISTRY` (metadata, assumption key, simulator, validator), `PRODUCT_MANIFEST`, `PRODUCT_IDS`, `getProductMeta(id)`. Single source of truth for product ids, labels, colors, validators, and sort order. |
...
**ETF Vorabpauschale** (`accumulation.ts` / `etfPayout.ts`): `projectAccumulation` receives an optional `etfVorabpauschale` config and accumulates `cumulativeVorabpauschale` on each row. `afterTaxInvestmentCapital` and `etfPayoutSchedule` accept this to reduce the taxable gain at exit/cost-basis schedule. Only wired for ETF products.
...
**Product registry** (`productRegistry.ts`): `PRODUCT_REGISTRY` is the single source of truth for product display metadata, assumption key, simulator, and validator. Each product module in `src/engine/products/` exports a `metadata` constant; the registry aggregates them. Use `getProductMeta(productId)` in UI code instead of local color/order maps.
