...
Verified against the current repo on 2026-05-05: transfer events are modeled in `src/domain/instances.ts`, fed through `portfolioAdapter.ts`, supported by accumulation capital injections/withdrawals, and exercised by certified and surrender-reinvest transfer tests. This issue appears shipped; remaining Group G defects live under `.scratch/group-g-qa/issues/`.
