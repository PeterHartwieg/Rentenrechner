...
Verified against the current repo on 2026-05-05: `src/engine/portfolioCombine.ts` aggregates per-instance results through shared retirement tax and KV/PV logic, and `src/app/useCombineSimulation.ts` wires it into combine mode, with cross-instance and empty-baseline tests. This issue appears shipped; remaining Group G defects live under `.scratch/group-g-qa/issues/`.
