...
Verified against the current repo on 2026-05-05: `src/app/recommender.ts`, `RecommenderCard.tsx`, and `LueckeSchliessenModal.tsx` implement ranked marginal-budget candidates, sorting, atoms, and what-if materialization hooks. Remaining recommender QA defects, including current Lücke-schließen and save-plan behavior, live under `.scratch/group-g-qa/issues/`.
