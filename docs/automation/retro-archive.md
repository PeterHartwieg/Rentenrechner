...
- Single-stage shortcut applied: no Stage 1 handoff, empty `git log origin/main..HEAD`, labels matched `area:copy`. Fetched issue body directly, identified the affected file.
- The heading `Mit ${retirementAge}, einmalig` lives in `src/features/vergleich-detail/vergleichDetailRows.ts:277` inside `buildKapitalSection`. The section shows the capital balance at retirement — "einmalig" incorrectly implies a lump-sum payout. Fixed to `Kapital`.
- Four test files assert the old heading string and needed updating: `vergleichDetailRows.test.ts` (3 assertions), `VergleichDetailPage.test.tsx` (1 assertion + 1 comment), `VergleichDetailCardSection.test.tsx` (1 fixture heading). `PrintReport.test.tsx` does not assert this heading text.
- OG images under `public/og/` are regenerated by the `prerender` step of `npm run build`; they appear as uncommitted changes after `npm run verify` but are not part of the fix.
