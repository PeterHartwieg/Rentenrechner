...
**Implementation status (2026-09-08): all three immediate fixes are implemented on `codex/input-followup-plans`.** Delivery followed the recommended order (plan 3 → 2 → 1), plus four corrections from the supervising review (legacy-anchor display resolution, neutral tax-class labels, single currency + anchor-only hint on the bAV metric, `modellbasierte` typo). Both independent reviews of candidate 66d3cd3 are in: Grok 4.6 found no blockers; Opus 5 raised one confirmed P1 — the § 1 copy unconditionally excluded joint retirement taxation although combine mode wires it — fixed in this revision together with the `<label>`-without-control markup on the bAV metric. `npm run verify` passes (208 test files, 3914 tests). Re-confirmation by both reviewers is pending. Open follow-ups: the scenario-library hardening item in plan 2, the household/church-tax feature in plan 1, and the pre-existing shared-pension-card baseline issue described under "Deferred follow-ups" below.

## 1. Make personal-information controls match supported calculations

**Implemented.** § 1 now carries the Steuerklasse I–VI select bound to `profile.taxClass` (neutral labels, salary-phase hint scoped to Lohnsteuer/Förderwirkung); the Familienstand/Bundesland dropdowns, Kirchensteuer checkbox, and their effect claims are removed; the § Person aside and the Datenhaltung disclosure state only what persists and what the engine consumes. After the Opus-5 review the scope copy no longer claims joint retirement taxation is unsupported — combine mode wires it (`CombineHaushaltSection` → `baseline.partner`); the honest exclusions kept are church tax and "no complete household tax return". Saved `churchTax` values still load untouched. Regression coverage in `AngabenPage.test.tsx` (persistence in both modes, removed controls, truthful scope copy in compare AND combine, legacy `churchTax: true` survival, funding-vs-statutory-pension split, no partner created).
...
- **Shared pension card ignores `pensionBaselineType` (pre-existing, Opus 5 P2).** `grvCard.ts` hardcodes `DRV · Schicht 1 · Pflicht` / "Gesetzliche Rentenversicherung" and always renders the Entgeltpunkte rows and "Heutiger Rentenwert (West)", while `GRVInputs` supports `versorgungswerk`, `beamtenpension`, and `none` baselines — for those the card names the wrong system and shows rows the calculation does not use. Out of scope for these corrections; needs its own fix (card kind/title/rows keyed off `statutoryPension.pensionBaselineType`) plus updated rendering tests in both panel branches.
- **Scenario-library harmonization (plan 2 hardening item)** — see "Separate hardening item" above.
- **Full household and church-tax support (plan 1 follow-on feature)** — see "Follow-on feature" above.
